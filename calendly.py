...
# ==========================================
# 📋 FORMULAIRE DE COMMANDE AMÉLIORÉ ET CORRIGÉ
# ==========================================
def formulaire_commande(panier, total_articles):
    ...
    with st.form("commande_form", clear_on_submit=False):
        ...
        if mode_livraison != "📍 Retrait Gonesse":
            ...
            with adresse_col1:
                rue = st.text_input("Adresse complète*", placeholder="123 Avenue des Champs-Élysées", key="rue_input")
                ville = st.text_input("Ville*", placeholder="Paris", key="ville_input")
        ...
        with col1:
            nom = st.text_input("Nom & Prénom*", placeholder="Jean Dupont", key="nom_input")
            telephone = st.text_input("Téléphone*", placeholder="06 12 34 56 78", key="tel_input")
        
        with col2:
            email = st.text_input("Email*", placeholder="jean.dupont@email.com", key="email_input")
            instagram = st.text_input("Votre Instagram", placeholder="@votrepseudo", key="insta_input")
        ...
        # Validation
        cgu = st.checkbox("J'accepte les conditions générales de vente*", value=False, key="cgu_check")
    ...
    if submitted:
        ...
        # Validation champs obligatoires de base
        if not nom:
            erreurs.append("Nom & Prénom")
        if not telephone:
            erreurs.append("Téléphone")
        if not email:
        ...
        if erreurs:
            st.error(f"❌ Champs manquants : {', '.join(erreurs)}")
            return False
