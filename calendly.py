...
THEME = {
    "nom": "Standard",
    "bg_color": "#FFFFFF",  # Blanc pur pour meilleure visibilité
    "main_color": "#D4AF37",
    "secondary_color": "#8B7355",
    "text_color": "#2D1E12",  # Marron très foncé pour contraste
    "icon": "🌹"
}
...
/* ZONES DE TEXTE PERSONNALISÉES */
.text-option {{
    background-color: #FFF8E1 !important;
    border: 2px dashed {THEME['main_color']} !important;
    border-radius: 10px;
    padding: 15px;
    margin: 10px 0;
}}
...
# ACCESSOIRES BOUQUET AVEC ZONES TEXTE
ACCESSOIRES_BOUQUET = {
...
# ACCESSOIRES BOX CHOCOLAT AVEC ZONES TEXTE
...
def configurer_bouquet():
    ...
    # Accessoires avec zones texte conditionnelles
    st.subheader("✨ Accessoires optionnels")
    st.write("Cochez les options souhaitées :")
    
    options_choisies = []
    details_personnalisation = {}  # Stocke les textes personnalisés
    
    # Afficher les accessoires en grille
    cols = st.columns(3)
    ...
    for idx, (nom, details) in enumerate(items):
        ...
        with cols[col_idx]:
            ...
            if checked:
                ...
                # Afficher zone texte si nécessaire
                if details.get("zone_texte", False):
                    st.markdown('<div class="text-option">', unsafe_allow_html=True)
                    texte = st.text_input(
...
def configurer_box_chocolat():
    ...
    if st.button(f"🍫 AJOUTER CETTE BOX CHOCOLAT - {prix_total}€", use_container_width=True, type="primary"):
        description = f"Box chocolat {taille} | Chocolats: {', '.join(chocolats) if chocolats else 'Aucun'}"
...
def formulaire_commande(panier, total_articles):
    ...
    if submitted:
        ...
        # Construction du message RÉCAPITULATIF AMÉLIORÉ
        lignes_articles = "\n".join([
...
{f"• Adresse : {rue}, {code_postal} {ville}" + (f" ({complement})" if complement else "") if mode_livraison != "📍 Retrait Gonesse" else "• Adresse : Retrait sur place"}

💰 MONTANTS
• Sous-total articles : {total_articles}€
• Frais de livraison : {frais_livraison}€
• TOTAL COMMANDE : {total_final}€
• 🔐 ACOMPTE À PAYER (40%) : {acompte:.2f}€
• 📦 SOLDE À LA LIVRAISON : {total_final - acompte:.2f}€
...
def confirmation_commande():
    ...
    Votre commande a été enregistrée. Voici les prochaines étapes :
    """)
    
    # Téléchargement du récapitulatif
    st.download_button(
        label="📥 TÉLÉCHARGER LE RÉCAPITULATIF DÉTAILLÉ",
        data=cmd["message"],
        file_name=f"commande_sun_creation_{date.today().strftime('%Y%m%d_%H%M')}.txt",
        mime="text/plain",
        help="Conservez ce fichier pour votre suivi"
    )
    
    # Affichage du récapitulatif dans l'interface
    ...
    **📋 PROCÉDURE À SUIVRE :**
    ...
    6. **Votre commande sera préparée** avec soin
